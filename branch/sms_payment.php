...
/*Payment Process For Purchase SMS*/
if (isset($_GET['submit_payment']) && !empty($_GET['submit_payment']) && isset($_GET['package_id']) && !empty($_GET['package_id'])) {
    $amount = $_GET['amount']; 
    ...
    if ($id_token) {
        $paymentResponse = createPayment($base_url, $id_token, $app_key, $callbackURL, $amount, $package_id);
...
/*Create Payment*/ 
function createPayment($base_url, $id_token, $app_key, $callbackURL, $amount, $package_id) {
    $request_data = [
        'callbackURL' => $callbackURL,
        'payerReference' => $package_id,
        'mode' => '0011',
        'amount' => $amount,
        'intent' => 'sale',
        'currency' => 'BDT',
