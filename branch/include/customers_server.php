...
if (isset($_POST['getPackagePrice'])) {
    ...
    $pop_id = $_POST['pop_id'];
    if ($allArea = $con->query("SELECT * FROM branch_package WHERE pkg_id='$packageId' AND pop_id=$pop_id   ")) {
        while ($rowsssss = $allArea->fetch_array()) {
...
if (isset($_POST['addCustomerData'])) {
    ...
    if ($allPack = $con->query("SELECT * FROM branch_package WHERE pkg_id=$package")) {
