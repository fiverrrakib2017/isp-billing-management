        ...
        $("#customers_table").DataTable();
                                        ...
                                        <table id="customers_table" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                            ...
                                            <tbody id="customer-list">
												...
												// Search by status (Online)
												if($srch_pop_id !="" && $srch_area_id =='all' && $customer_sttssrch =='online')
												 {
													 $sql = "SELECT customers.* FROM customers
															INNER JOIN radacct
															ON customers.username = radacct.username
															WHERE customers.pop = '$srch_pop_id' AND radacct.acctstoptime IS NULL";	 
												 }
												...
												// Search by status Expired (2)
												if($srch_pop_id !="" && $srch_area_id =='all' && $customer_sttssrch =='expired')
												 {
													 $sql = "SELECT * FROM customers WHERE pop='$srch_pop_id' AND status='2'";	 
												 }
												 
												 // Search by status Expired (0)
												if($srch_pop_id !="" && $srch_area_id =='all' && $customer_sttssrch =='disabled')
												 {
													 $sql = "SELECT * FROM customers WHERE pop='$srch_pop_id' AND status='0'";	 
												 }
												 // Search by status Expired (1)
												if($srch_pop_id !="" && $srch_area_id =='all' && $customer_sttssrch =='active')
												 {
													 $sql = "SELECT * FROM customers WHERE pop='$srch_pop_id' AND status='1'";	 
												 }
